     ...
     $NO=rand();
       ...
       if($conn->connect_error)
          ...
          else
          {
              ...
              $stmt=$conn->prepare("INSERT INTO booking(passenger_name,telephone,email,boarding_place,Your_destination,booked_seat,booking_id) VALUES(?,?,?,?,?,?,?)");
              //table3 is the table name//

              $stmt->bind_param("sssssii",$passenger,$tel,$email,$board_place,$desti,$seat,$NO);

              $stmt->execute();
              
                         echo ("<script LANGUAGE='JavaScript'>
                window.alert('Succesfully added!!! and Your Ticket no is $NO');
                window.location.href='TuserAddPay.php';
                </script>");
